...
class MedicalRecordsAPI{ 
  ...
  Future<void> getProceduralRecords({required BuildContext context,required String patientId}) async {
    var bearerToken = await flutterSecureStorage.read(key: "BEARERTOKEN");
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get("$PROCEDURAL_RECORDS_URL?patientid=$patientId",options : dioOptions);
      if(response.statusCode == 200) {
        print(response.data);
        return response.data['BODY'];
      }
  ...
  Future<void> getAllDisplayRecords({required BuildContext context,required String recordId,required String deptId}) async {
    var bearerToken = await flutterSecureStorage.read(key: "BEARERTOKEN");
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get("$DISPLAY_RECORDS_URL?recordid=$recordId&deptid=$deptId",options : dioOptions);
      if(response.statusCode == 200) {
        print(response.data);
        return response.data['BODY'];
      }
  ...
  Future<void> getAppointmentsRecords({required BuildContext context,required String recordId,required String deptId,required String appointmentId}) async {
    var bearerToken = await flutterSecureStorage.read(key: "BEARERTOKEN");
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get("$APPOINTMENT_RECORDS_URL?recordid=$recordId&deptid=$deptId&aid=$appointmentId",options : dioOptions);
      if(response.statusCode == 200) {
        print(response.data);
        return response.data['BODY'];
      }
