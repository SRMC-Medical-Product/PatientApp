...
class RaisedIssuesAPI{ 
 
  Future<void> getRaisedIssues({required BuildContext context}) async {
    var bearerToken = await flutterSecureStorage.read(key: "BEARERTOKEN");
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get(PATIENT_TICKETS,options : dioOptions);
      ...
      if (e.type == DioErrorType.sendTimeout) {
        ...
        print("${e.message} + ${e.type} in raise issues api");
        //Logout
  ...
  postRaiseIssue({required BuildContext context,required String appointmentId,required String title,required String description}) async {
    var bearerToken = await flutterSecureStorage.read(key: "BEARERTOKEN");
    dio.options.headers["authorization"] = "Bearer $bearerToken";
    ...
    FormData data = FormData.fromMap({
     ...
     try {
      Response response = await dio.post(PATIENT_TICKETS,data:data,options : dioOptions);
      if(response.statusCode == 200) {
        print(response.data);
        ScaffoldMessenger.of(context).showSnackBar(customSnackSuccessBar(context, "Your query has been raised. Wait for our support"));
        return true;
      }
    } on DioError catch (e) {
      if (e.type == DioErrorType.sendTimeout) {
