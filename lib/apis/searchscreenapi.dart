...
class SearchAPI{ 
  ...
  Future<void> getAllCategories({required BuildContext context}) async {
    ...
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get(ALL_CATEGORIES_SCREEN_URL,options : dioOptions);
      ...
      if (e.type == DioErrorType.sendTimeout) {
        ...
        print("${e.message} + ${e.type} in search screen api");
        //Logout
  ...
  Future<dynamic> performSearch({required BuildContext context,required String searchType,String? searchQuery,String? doctorSpecialist,String? doctorExp,String? doctorGender}) async {
    ...
    dio.options.headers["authorization"] = "Bearer $bearerToken";
     try {
      Response response = await dio.get("$SEARCH_DOCTORS_URL?search_type=$searchType&query=$searchQuery&specialist=$doctorSpecialist&exp=$doctorExp&gender=$doctorGender",options : dioOptions);
      if(response.statusCode == 200) {
        print(response.data);
        return response.data['BODY'];
      }
